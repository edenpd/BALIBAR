...
function create_type_pie_chart(data, wanted_element) {
    ...
    g.append("path")
        .attr("d", arc)
        .style("fill", function (d) { return color(d.data.typeName); });
    ...
    g.append("text")
        ...
        .attr("text-anchor", "middle")                          //center the text on it's origin 
        .attr("fill", "white")                          
        .text(function (d, i) { return data[i].typeName + " : " + data[i].reservations; });        //get the label from our original data array
};
